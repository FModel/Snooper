...
layout(std430, binding = 3) restrict readonly buffer SplineParameters
{
...
float GetAxisValueRef(uint forwardAxis, vec3 pos)
{
    switch (forwardAxis)
    {
        case 0: return pos.x;
...
void SetAxisValueRef(uint forwardAxis, inout vec3 pos, float v)
{
    switch (forwardAxis)
    {
...
vec3 SplineEvalTangent(SplineMeshParams params, float a)
{
...
vec3 SplineEvalDir(SplineMeshParams params, float a)
{
    return getSafeNormal(SplineEvalTangent(params, a));
}

vec3 SplineEvalPos(SplineMeshParams params, float a)
{
    float a2 = a * a;
    float a3 = a2 * a;
...
vec3 ComputeRatioAlongSpline(SplineMeshParams params, float distanceAlong)
{
    float alpha = 0.0;
    float minT = 0.0;
    float maxT = 1.0;
    ...
    if (bHasCustomBoundary)
    {
        ...
        float boundMin = GetAxisValueRef(params.ForwardAxis, params.MeshOrigin.xzy - params.MeshBoxExtent.xzy);
        float boundMax = GetAxisValueRef(params.ForwardAxis, params.MeshOrigin.xzy + params.MeshBoxExtent.xzy);
    ...
    {
        float meshMinZ = GetAxisValueRef(params.ForwardAxis, params.MeshOrigin.xzy) - GetAxisValueRef(params.ForwardAxis, params.MeshBoxExtent.xzy);
        float meshRangeZ = 2 * GetAxisValueRef(params.ForwardAxis, params.MeshBoxExtent.xzy);
...
mat4 CalcSliceTransformAtSplineOffset(SplineMeshParams params, vec3 computed)
{
