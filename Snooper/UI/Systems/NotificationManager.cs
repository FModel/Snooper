...
public class NotificationManager
{
    ...
    public void PushNotification(string title, Func<string> message, float duration = 3f, Func<float>? progress = null)
    {
        if (!_shown.Add(title)) return;
    ...
    public void DrawNotifications()
    {
        ...
        for (var i = _notifications.Count - 1; i >= 0; i--)
        {
            ...
            ImGui.TextWrapped(n.Message.Invoke());
            ImGui.PopFont();
            if (n.Progress != null)
            {
                ImGui.Spacing();
    ...
    private class Notification(string title, Func<string> message, float duration, Func<float>? progress)
    {
        public readonly string Title = title;
        public readonly Func<string> Message = message;
